    ...
    <Row class="padding-5-percent" style="padding-bottom:10px;">
        <i-col>
          ...
          <div>
            <Row class="interval">
              <i-col span="8">
                <label id="user_id" style="display: none;">{{detailData.id}}</label>
                <label>申请人：</label>
      ...
      <div slot="footer">
        <Row>
              ...
              <i-col span="21">
                <i-input v-model = 'finalOpinion' type="textarea" :autosize="{minRows: 2,maxRows: 5}"  placeholder="请输入您的终审意见"></i-input>
                <div v-if="finalOpinion===''" id="warning_block" class="warning-msg" style="float:left;color: #f2565c;display: none;">请输入退回意见</div>
              </i-col>
          </Row>
          <Row style="text-align: center;margin-top: 10px;">
            <Button @click="choose('cancel')">取消</Button>
            <Button @click="choose('refuse')">退回</Button>
  ...
  methods: {
    ...
    submitSendback (type) {
      ...
      if (this.finalOpinion === '') {
        document.getElementById('warning_block').style.display = 'inline-block'
        return
      }
      switch (type) {
        case 'ok':
