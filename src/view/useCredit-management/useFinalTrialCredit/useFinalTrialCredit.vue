<template>
  <div>
      ...
      <i-col>
        ...
        <Modal v-model="showModal" width="600" :mask-closable="false" class-name="vertical-center-modal">
          ...
          <div>
            <Row class="interval">
              <i-col span="8">
                <label id="userFinalApplicationId" style="display: none;">{{detailData.id}}</label>
                <label>申请人：</label>
          ...
          <div slot="footer">
            ...
            <Row style="margin-top: 10px">
              <i-col span="24" style="text-align: left">
                <label>初审建议：</label>
                <span>{{detailData.firstCheckOpinion}}</span>
              </i-col>
            </Row>
            <Row style="margin-top: 10px">
              ...
              <i-col span="15">
                <i-input v-model = 'finalOpinion' type="textarea" :autosize="{minRows: 2,maxRows: 5}"  placeholder="请输入您的建议"></i-input>
                <label v-if="finalOpinion===''" id="finalOpinion-alert" style="float: left;color: #f2565c;display: none;">请输入您拒绝的原因！</label>
              </i-col>
            </Row>
        ...
        <Modal v-model="modalConfirm" :mask-closable="false" class-name="vertical-center-modal">
          <Row>
            ...
            <i-col span="16" style="font-size: 16px;">
              ...
              <p><span style="color: #ed1c24;">{{detailData.applicationSum}}</span>&nbsp;元，初审通过？</p>
            </i-col>
          </Row>
          <div slot="footer">
            <Row style="text-align: center;margin-top: 10px;">
        ...
        <!--<Modal v-model="modalSendBack" :mask-closable="false" class-name="vertical-center-modal">
          <Row>
            ...
            <i-col span="16" style="font-size: 16px;">
              <p style="line-height: 2;">您确认将用户{{detailData.applicant}}的授信申请退回经办吗?</p>
            </i-col>
          </Row>
          <div slot="footer">
            <Row style="text-align: center;margin-top: 10px;">
    ...
    <Row class="padding-5-percent">
      <i-col>
        <Page :total="dataCount" :page-size="pageSize" style="float: right;margin-top: 5px;" show-elevator show-sizer  show-total/>
      </i-col>
    </Row>
  </div>
</template>
...
export default {
                  ...
                  click: () => {
                    ...
                    this.request('get', 'http://localhost:8088/credit/get/' + dataId)
                      .then((data) => {
                        ...
                        this.detailData['firstCheckOpinion'] = data.firstCheckOpinion
                        this.showModal = true
                      }).catch((response) => {
                        this.$Modal.warning({
                          title: 'response',
  ...
  methods: {
    submit (type) {
      switch (type) {
        case 'ok':
          let userFinalApplicationId = document.getElementById('userFinalApplicationId').innerHTML
          let data = {}
          data['credit'] = {'id': userFinalApplicationId}
          data['approvalResult'] = '1'
          data['approvalType'] = '2'
          if (this.finalOpinion !== '') {
            data['approvalOpinion'] = this.finalOpinion
          }
    ...
    choose (type) {
      switch (type) {
        ...
        case 'refuse':
          if (this.finalOpinion === '') {
            document.getElementById('finalOpinion-alert').style.display = 'inline-block'
            return
          }
          let userFinalApplicationId = document.getElementById('userFinalApplicationId').innerHTML
          let data = {}
